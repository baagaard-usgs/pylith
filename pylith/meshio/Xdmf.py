...
class Xdmf(object):
    ...
    def write(self, filenameH5, filenameXdmf=None, verbose=False):
        ...
        if not os.path.isfile(filenameH5):
            raise IOError(
                "Cannot create Xdmf file for HDF5 file '%s'. File not found."
                % filenameH5
            )
        ...
        if self._getSpaceDim() == 1:
            print(
                "WARNING: Xdmf grids are not defined for 1-D domains.\n"
    ...
    def _getXdmfCellType(self, cells):
        ...
        if 0 == cellDim and 1 == numCorners:
            ...
            cellType = "Unknown"
            print(
                "WARNING: Unknown cell type with %d vertices and dimension %d."
                % (numCorners, cellDim)
            )
        return cellType
    ...
    def _getXdmfVectorFieldType(self, vectorFieldString):
        ...
        if type(vectorFieldString) == str:
            ...
            raise TypeError(
                f"Unknown type for vector field string ({type(vectorFieldString)})."
            )
            ...
            if group in self.h5:
                ...
                for name, dataset in vfields.items():
                    ...
                    if "vector_field_type" in dataset.attrs:
                        ...
                        print(
                            "WARNING: Field '%s/%s' dataset missing 'vector_field_type' attribute. Guessing vector field type."
                            % (
                                group,
                                name,
                            )
    ...
    def _openDomain(self, cells, vertices):
        ...
        if 3 == spaceDim:
            ...
            self._close()
            raise ValueError(
                "Unexpected spatial dimension %d when writing domain vertices."
                % spaceDim
            )

    def _closeDomain(self):
        """Close domain element."""
        self.file.write("  </Domain>\n")
    ...
    def _openTimeGrid(self):
        ...
        self.file.write('      <Grid Name="domain" GridType="Uniform">\n')
    ...
    def _writeGridFieldComponent(self, field, iTime, iComponent):
        ...
        if field.vectorFieldType == "Vector":
            ...
            if spaceDim == 2:
                ...
                self._close()
                raise ValueError(
                    "Unexpected spatial dimension %d for field component names."
                    % spaceDim
                )
            componentName = field.name + components[iComponent]
            ...
            self._close()
            raise ValueError(
                "Unexpected vector field type '%s' for field component names."
