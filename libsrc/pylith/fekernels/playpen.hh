...
struct VectorField {
    ...
    PETSC_DEVICE inline PylithScalar operator()(PylithInt i) const {
        return value[i];
    }

    PETSC_DEVICE inline PylithScalar dot(PylithInt i) const {
        return value_t[i];
    }

    // grad[i*dim+j] = du_i/dx_j
    PETSC_DEVICE inline PylithScalar grad_ij(PylithInt i,
                                             PylithInt j) const {
        return grad[i*dim+j];
    }

    PETSC_DEVICE inline PylithScalar symGrad(PylithInt i,
                                             PylithInt j) const {
        return 0.5*(grad[i*dim+j] + grad[j*dim+i]);
    }

    PETSC_DEVICE inline PylithScalar trace() const {
        PylithScalar tr = 0.0;
    ...
    PETSC_DEVICE inline PylithScalar traceDot() const {
        PylithScalar tr = 0.0;
        for (PylithInt i = 0; i < dim; i++) {
            tr += value_t[i*dim+i];
        }
...
struct ScalarField {
    ...
    PETSC_DEVICE inline PylithScalar operator()() const {
        return value[0];
    }

    PETSC_DEVICE inline PylithScalar dot() const {
        return value_t[0];
    }

    PETSC_DEVICE inline PylithScalar grad_i(PylithInt i) const {
        return grad[i];
    }
...
struct IsotropicLinearElasticity {
    ...
    struct Solution {
        ...
        template <PylithInt dim>
        PETSC_DEVICE static inline Unpacked<dim>unpack(const PylithInt sOff[],
    ...
    struct Auxiliary {
        ...
        PETSC_DEVICE static inline Unpacked unpack(const PylithInt aOff[],
    ...
    // f0_i = -rho * g_i
    template <PylithInt dim>
    PETSC_DEVICE static inline void f0(const PylithInt dim_,
    ...
    template <PylithInt dim>
    PETSC_DEVICE static inline void f1(const PylithInt dim_,
                                       const PylithInt numS,
                                       const PylithInt numA,
                                       const PylithInt sOff[],
                                       const PylithInt sOff_x[],
    ...
    template <PylithInt dim>
    PETSC_DEVICE static inline void Jf3_uu(const PylithInt dim_,
                                           const PylithInt numS,
                                           const PylithInt numA,
                                           const PylithInt sOff[],
                                           const PylithInt sOff_x[],
    ...
    template <PylithInt dim>
    PETSC_DEVICE static inline void f0_inertial(const PylithInt dim_,
                                                const PylithInt numS,
                                                const PylithInt numA,
                                                const PylithInt sOff[],
                                                const PylithInt sOff_x[],
    ...
    template <PylithInt dim>
    PETSC_DEVICE static inline void Jf0_uu_inertial(const PylithInt dim_,
                                                    const PylithInt numS,
                                                    const PylithInt numA,
                                                    const PylithInt sOff[],
                                                    const PylithInt sOff_x[],
    ...
    template <PylithInt dim>
    PETSC_DEVICE static inline void f0_traction(const PylithInt dim_,
                                                const PylithInt numS,
                                                const PylithInt numA,
                                                const PylithInt sOff[],
                                                const PylithInt sOff_x[],
...
struct AuxiliaryLayout {
    ...
    static constexpr bool has(AuxFlags f) {
        return (flags & f) != 0;
    }
    ...
    template <PylithInt dim>
    PETSC_DEVICE static inline Unpacked<dim>unpack(const PylithInt aOff[],
                                                   const PylithInt aOff_x[],
                                                   const PylithScalar a[],
                                                   const PylithScalar a_t[],
                                                   const PylithScalar a_x[]) {
...
template <PylithInt dim, AuxFlags flags>
void registerKernels(PetscDS ds,
                     PylithInt field,
                     bool dynamic);

// Runtime dispatch: branch on dim and flags once at setup
void
setupElasticity(PetscDS ds,
                PylithInt dim,
                PylithInt field,
                bool hasGravity,
                bool hasBodyForce,
...
SolutionLayout
solution_layout(const ProblemConfig& cfg) {
    uint32_t v = 0;
...
AuxLayout
aux_layout(const ProblemConfig& cfg) {
    uint32_t v = 0;
...
template<typename Dim, typename StrainModel, typename StressModel>
void
register_config(std::unordered_map<KernelKey, KernelFn, KernelKeyHash>& map) {
...
void
KernelRegistry::register_one<Config<Dim, S, T> >() {
    register_config<Dim, S, T>(map_);
}


std::string
unsupported_msg(const KernelKey& k) {
